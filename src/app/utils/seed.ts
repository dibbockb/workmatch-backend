...
async function main() {
	...
	const userIndex = adminCount + clientCount;
	...
	for (let i = 0; i < jobCount; i++) {
		...
		jobs.push({
			...
			selectedProposalId:
				jobStatuses[i % jobStatuses.length] !== JobStatus.OPEN
					? crypto.randomUUID()
					: null,
			createdAt,
			deletedAt:
				jobStatuses[i % jobStatuses.length] === JobStatus.CANCELLED
					? new Date()
					: null,
		});
	}
	...
	for (let i = 0; i < proposalCount; i++) {
		...
		proposals.push({
			...
			proposedPrice: Number(
				(
					rand() * (Number(job.budgetMax) - Number(job.budgetMin)) +
					Number(job.budgetMin)
				).toFixed(2),
			),
			proposedTimeline: Math.floor(rand() * 30) + 7,
			approachDescription:
				approachDescriptions[i % approachDescriptions.length],
			status,
			submittedAt,
			respondedAt:
				status !== ProposalStatus.PENDING ? new Date(submittedAt) : null,
		});
	}
	...
	for (let i = 0; i < counterOfferCount; i++) {
		...
		const clientId =
			jobs.find((j) => j.id === proposal.jobId)?.clientId || clientUserIds[0];
		const offeredBy =
			rand() > 0.5 ? CounterOfferOrigin.CLIENT : CounterOfferOrigin.FREELANCER;
		...
		counterOffers.push({
			...
			offeredBy,
			proposedPrice: Number(
				(Number(proposal.proposedPrice) * (0.8 + rand() * 0.4)).toFixed(2),
			),
			proposedTimeline: proposal.proposedTimeline + Math.floor(rand() * 10) - 5,
			message:
				offeredBy === CounterOfferOrigin.CLIENT
					? "We can adjust the timeline if the price fits our budget."
					: "I can adjust the timeline to meet your deadline while maintaining fair compensation.",
		...
		contracts.push({
			...
			endDate:
				status === ContractStatus.COMPLETED
					...
					: null,
			deliverables:
				"Full project delivery with source code, documentation, and testing results.",
		});
	}
		...
		payments.push({
			...
			stripeSessionId:
				status === PaymentStatus.SUCCEEDED
					? `cs_test_${crypto.randomUUID().slice(0, 24)}`
					: null,
	...
	for (let i = 0; i < counterOffers.length; i += 10) {
		await prisma.counterOffer.createMany({
			data: counterOffers.slice(i, i + 10),
		});
	}
	...
	console.log(
		`Seeded: ${users.length} users, ${clients.length} clients, ${freelancers.length} freelancers, ${jobs.length} jobs, ${proposals.length} proposals, ${counterOffers.length} counterOffers, ${contracts.length} contracts, ${payments.length} payments, ${reviews.length} reviews, ${auditLogs.length} auditLogs`,
	);
